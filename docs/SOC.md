...
| IRIS: admin menus work but a case says *Access denied while trying to access case #1* | IRIS keeps a second control - the per-case ACL in `user_case_effective_access` (1 = deny_all, 2 = read_only, 4 = full_access). It is only recomputed when membership changes through the UI/API, so a group granted directly in the database leaves the user at deny_all | `python3 scripts/iris-sync-users.py` - it now recomputes effective case access from the group grants |
| IRIS: logged in via SSO but every page says **ACCESS DENIED** | IRIS auto-creates the SSO user with **no group**, and since v2.4.0 the default case access is `deny_all` | `python3 scripts/iris-sync-users.py` - it maps each SSO account to an IRIS group (Administrators / Analysts). Users only appear after their first login, so run it once afterwards |
| IRIS: user created with e-mail equal to the login (no domain) | the IdP token carried no `email` claim, so IRIS fell back to the username | harmless - the sync script matches on either form; to fix properly ensure the client requests the `email` scope and the Keycloak user has an e-mail set |
